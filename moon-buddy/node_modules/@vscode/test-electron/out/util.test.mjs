...
import { downloadDirToExecutablePath, insidersDownloadDirMetadata, insidersDownloadDirToExecutablePath, } from './util.js';
describe('insidersDownloadDirMetadata (win32)', () => {
    ...
    const writeProductJson = async (relativeDir, commit) => {
...
describe('downloadDirToExecutablePath (darwin)', () => {
    ...
    const macosDirFor = (bundleName) => join(dir, bundleName, 'Contents', 'MacOS');
    const writeInfoPlist = async (bundleName, cfBundleExecutable) => {
...
`;
    ...
    const touchExecutable = async (bundleName, executableName) => {
    ...
    // no coverage.
    test.skipIf(process.platform === 'win32')('Stable: transitional symlink layout — plist name wins over Electron symlink', async () => {
    ...
    test.skipIf(process.platform === 'win32')('Stable: tier 2 ignores the compatibility symlink and picks the real binary', async () => {
    ...
    test('Stable: falls back to Electron when Info.plist lacks CFBundleExecutable and MacOS/ is empty', async () => {
        ...
        await fs.mkdir(contentsDir, { recursive: true });
        await fs.writeFile(join(contentsDir, 'Info.plist'), '<?xml version="1.0"?><plist version="1.0"><dict></dict></plist>');
        const exePath = downloadDirToExecutablePath(dir, 'darwin-arm64');
        expect(exePath).to.equal(join(macosDirFor('Visual Studio Code.app'), 'Electron'));
    });
    test('Stable: rejects CFBundleExecutable containing path traversal', async () => {
        ...
        const contentsDir = join(dir, bundleName, 'Contents');
        await fs.mkdir(contentsDir, { recursive: true });
        await fs.writeFile(join(contentsDir, 'Info.plist'), '<?xml version="1.0"?><plist version="1.0"><dict>' +
